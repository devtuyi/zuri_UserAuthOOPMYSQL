...
- ## Task `2` UserAuthentication Logic Class
  ...
  - Create a classes called UserAuth which has the following
    ...
    - Has the following methods
      - `confirmPasswordMatch($password, $confirmPassword)`: Takes in the two passwords passed from the form and checks if they are equal(If equal return true) else returns false
